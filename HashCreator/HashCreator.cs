...
namespace HashCreatorNS
{
    public class HashCreator
    {

        private HashDictionaryNS.HashDictionary hashDic;
        private HashDictionaryNS.HashDictionary patternTestHashDic;
        ...
        public HashCreator(HashDictionaryNS.HashDictionary hasher, Hasher.HasherType hasherType)
        {
            this.hashDic = hasher;
		...
		public void hashFilename(string filename, out uint ph, out uint sh)
		{
			Hasher hasher = new Hasher(hasherType);
			hasher.Hash(filename, 3735928559u);
			ph = hasher.ph;
			sh = hasher.sh;
		}
        ...
        public void Patterns(object obj)
        {
			...
			patternTestHashDic = (HashDictionaryNS.HashDictionary)obj;
                ...
                if (active)
                {
                    ...
                    foreach (KeyValuePair<string, Boolean> file in foundNames)
                        if (file.Value == true)
                        {
                            ...
                            swf.WriteLine("{0:X8}" + HashDictionaryNS.HashDictionary.hashSeparator
                                + "{1:X8}" + HashDictionaryNS.HashDictionary.hashSeparator
